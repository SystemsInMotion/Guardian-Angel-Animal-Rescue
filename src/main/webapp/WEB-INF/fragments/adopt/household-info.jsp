...
<div class="module">
	<c:choose>
		<c:when test="${not empty pet.lastUpdate}">
			<div class="row">
				...
				<select name="livingSituation" id="livingSituation" class="required"><option
						value="">Select...</option>
	...
	<div class="row" id="householdMemberAgesDiv">
		<c:if test="${empty pet.lastUpdate}">
			<label class="inline">List the ages of each household member:</label>
			<input type="text" class="required">
		</c:if>
	</div>

	<div class="row" id="familyAwareDiv">
		<form:label for="familyAware" path="familyAware" cssErrorClass="error"
			cssClass="autoWidth">Is everyone in your household aware you are adopting a
				pet?</form:label>
		<div class="minorElem">
