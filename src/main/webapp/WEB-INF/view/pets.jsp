				...
				<div class="pets-thumbnails">
					<c:forEach items="${pets}" var="pet">
						<div class="pet-thumbnail">
                                ...
                                <h4><a href="/app/pet/${pet.id}">${pet.name}</a></h4>
                                <c:choose>
                                    <c:when test="${empty pet.media.photos}">
                                        <a href="/app/pet/${pet.id}"><img
                                                src="/images/no_image-128.png" title="${pet.name}" class="shadow" /></a>
                                    </c:when>
                                    <c:otherwise>
							...
							<%--<h4>${pet.name}</h4>
							<c:forEach items="${pet.media.photos.photo}" var="photo" varStatus="status">
								<c:if test="${photo.id eq 1 and photo.size eq 'pn' }">
									<a href="/app/pet/${pet.id}"><img src="${photo.value}" title="${pet.name}" class="shadow" /></a>
								</c:if>
							</c:forEach>--%>
						</div>
					</c:forEach>
