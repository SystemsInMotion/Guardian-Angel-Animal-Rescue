...
public class RemoteController {
	...
	public @ResponseBody
	PetfinderPetRecord doGetPet(@PathVariable(value = "petId") int animalId) {
		return petFinderService.getPet(BigInteger.valueOf(animalId), FORMAT_NULL);
	}
	
	@RequestMapping(value = "pets", method = GET)
	public @ResponseBody PetfinderPetRecordList doGetPets() {
		// TODO - check is cache is expired, if so refresh cache and return list from cache
		PetfinderPetRecordList shelterPets = petFinderService.shelterPets(SHELTER_ID_GAAR, ADOPTION_STATUS_NULL, RECORD_OFFSET_NULL, RETURN_COUNT_ALL, OUTPUT_NULL, FORMAT_NULL);
		return shelterPets;
	}
